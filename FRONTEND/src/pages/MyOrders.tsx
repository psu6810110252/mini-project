  ...
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        ...
        const res = await axios.get('http://localhost:3001/orders/my-orders', {
                  ...
                  <div key={item.id} style={{ display: 'flex', alignItems: 'center', gap: '15px', marginBottom: '10px' }}>
                    ...
                    <div style={{ width: '60px', height: '60px', background: '#eee', borderRadius: '5px', overflow: 'hidden', flexShrink: 0 }}>
                      {item.product && item.product.image ? (
                        <img src={`http://localhost:3001/uploads/${item.product.image}`} alt={item.product.title} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                      ) : (
